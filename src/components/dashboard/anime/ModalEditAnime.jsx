...
import { useUpdateAnime } from "../../../hooks/dashboard/useUpdateAnime";

export const ModalEditAnime = () => {

    const [titleInput, setTitleInput] = useState('')
    const [gendersArray, setgendersArray] = useState([]) 
    const [poster, setPoster] = useState('second')
    ...
    const animeEdit = useGlobalStore(state => state.animeEdit)
    const setAnimeEdit = useGlobalStore(state => state.setAnimeEdit)

    
    const {data: dataGenders, isLoading} = useGender()
    const {mutate, isPending, isError} = useUpdateAnime()
    
    useEffect(() => {
        if(animeEdit) {
            setTitleInput(animeEdit.name)
            setgendersArray(animeEdit.gender)
            setPoster(animeEdit.image)
        }
    }, [animeEdit])
    ...
    }, [animeEdit, gendersArray, setValue])
  ...
  return (
    <div className="w-full h-screen fixed flex justify-center items-center backdrop-blur-xs z-50" onClick={handleClose}>
        <div className="w-130 h-min-200 flex flex-col bg-surface-dark mx-auto md:mt-5 rounded-2xl p-5 animated-slideInTop" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between ">
                <h3 className="text-white text-lg font-semibold flex justify-center items-center gap-2">
                    <FaEdit size={25} className="text-purple-700" />
                    Editar anime
                </h3>
                ...
                isLoading || isPending
                    ...
                    :<form className="flex flex-col w-full h-full justify-between" autoComplete="off" onSubmit={onRegister}>
                        <div className="flex flex-col justify-center space-y-2">
                            ...
                            <input type="text" id="title" placeholder="Ingrese un título" className="p-2 text-white text-[16px] rounded-lg otuline-none focus:outline-none bg-surface-dark-highlight border border-stone-600 transition-all duration-300" value={titleInput} onChange={handleChangeTitle}/>
                            {
                        ...
                        <div className="flex flex-col justify-center mt-4 space-y-2">
                            ...
                            <input type="text" id="poster" placeholder="Añade una url de la imagen del poster" className="p-2 text-white text-[16px] rounded-lg otuline-none focus:outline-none bg-surface-dark-highlight border border-stone-600 transition-all duration-300 " value={poster} onChange={handleChangePoster} />
                            {
                                errors.poster && <p className="text-sm text-red-500">{errors.poster.message}</p>
                            }
                        </div>

                        <button type="submit" className='w-full flex justify-center items-center gap-1.5 py-2 px-3 my-4 bg-purple-700 hover:bg-purple-600 text-white font-bold rounded-lg cursor-pointer transition-all duration-300 self-end'>Guardar</button>
                    </form>
            }
